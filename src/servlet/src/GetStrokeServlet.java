...
public class GetStrokeServlet {
	...
	/** 道路クラス */
	ArrayList<String> _roadClazz = new ArrayList<>();
	...
	public GetStrokeServlet(HttpServletRequest request, HttpServletResponse response) {
		...
		_roadClazz = new ArrayList<String>(Arrays.asList(request.getParameter("roadClazz").split(",")));
//				Arrays.asList(a)request.getParameter("roadClazz");
		
		// strokeを取り出す(geomデータはこっちを使う？).
		_osmStrokeDataGeom = new OsmStrokeDataGeom();
		_osmStrokeDataGeom.startConnection();
		_osmStrokeDataGeom.insertStrokeData(_upperLeftLngLat, _lowerRightLngLat, _roadClazz);	// ストロークを切り出さない.
//		_osmStrokeDataGeom.cutOutStroke(_upperLeftLngLat, _lowerRightLngLat);	// 切り出したストロークを求める.
		...
		// ストロークの描画する数.
		//int drawingStrokeNum = _isGetAllStroke ? _osmStrokeDataGeom._geojson.size() : ROAD_DRAW_NUM;
		int drawingStrokeNum = _osmStrokeDataGeom._geojson.size();
		ArrayList<String> drawingosmStrokeDataJson = new ArrayList<>();
		for(int i=0; i<drawingStrokeNum; i++){
			drawingosmStrokeDataJson.add(_osmStrokeDataGeom._geojson.get(_orderedStrokeIndexArrayList.get(i)));
		}
