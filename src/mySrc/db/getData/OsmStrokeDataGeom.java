...
public class OsmStrokeDataGeom extends HandleDbTemplateSuper{
	 ...
	 * 範囲内のストロークを取り出す
	 */
	public void insertStrokeData(Point2D aUpperLeftLngLat, Point2D aLowerRightLngLat, ArrayList<String> roadClazz){
		...
		String clazzes = "";
		for(int i=0; i<roadClazz.size();i++){
			clazzes +="'"+roadClazz.get(i)+"',";
		}
		clazzes = clazzes.substring(0, clazzes.length()-1);
		try{
			String statement;
			statement = "select "+
					" id, length,"+
